...
def get_accuracy(source_emb, target_emb):
    ...
    for idw, (w1, w2) in enumerate(np.array(list(zip(source_emb, target_emb)))):
        matches_e1 = np.append(matches_e1, int(csls_bulk(w1, target_emb, neigh_e1[idw], neigh_e2) == idw))
        matches_e2 = np.append(matches_e2, int(csls_bulk(w2, source_emb, neigh_e2[idw], neigh_e1) == idw))
